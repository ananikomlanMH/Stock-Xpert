...
// Navigation sidebar control
    ...
    if (chart__stats_year !== null) {
        ...
        const data = {
            "Ventes direct": Array(12).fill().map(() => Math.floor(Math.random() * 0)),
            "Approvisionnements": Array(12).fill().map(() => Math.floor(Math.random() * 0)),
            "Dépenses": Array(12).fill().map(() => Math.floor(Math.random() * 0)),
            "Autres charges": Array(12).fill().map(() => Math.floor(Math.random() * 0)),
        };
        let datasets = [];
