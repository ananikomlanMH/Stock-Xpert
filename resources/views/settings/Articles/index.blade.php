    ...
    <div class="filter-form mt-25">
        ...
        <div class="right-content d-flex gap-1">
            ...
            <button class="btn-primary JS_Call_Url_Get_Form"
                    data-url="{{ \App\Components\Router\Router::route('settings.article.addForm') }}">Nouveau<i
    ...
    <div class="table-list table-bordered table-striped ">
        <table>
            ...
            <tr>
                ...
                <th>{!! $tableHelper->th('categories_id') !!}</th>
                <th>{!! $tableHelper->th('pv') !!}</th>
                <th class="w-80"></th>
            </tr>
            </thead>
            <tbody>
            @forelse($data as $item)
                <tr>
                    ...
                    <td>{{ \App\Helpers\NumberHelper\NumberHelper::CurrencyFormat($item->pv) }}</td>
                    <td class="d-flex gap-1">
                        <a href="{{ \App\Components\Router\Router::route('settings.article.editForm', ['id' => $item->id]) }}" class="link__table JS_Call_Url_Get_Form"><i class="fi-rr-edit"></i> Editer</a>

                        <form action="{{ \App\Components\Router\Router::route('settings.article.delete', ['id' => $item->id]) }}" method="post" class="deleteForm">
